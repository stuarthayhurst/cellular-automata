...
import { stepForward, computeCellState, countMooresNeighbours } from '../src/js/simulator.js';
...
describe('Simulator Functions', () => {
    ...
    describe('computeCellState()', () => {
        it('should return 1 when cell is dead and has exactly 3 alive neighbors', () => {
            expect(computeCellState(0, 3)).to.equal(1);
        });

        it('should return 1 when cell is alive and has 2 or 3 alive neighbors', () => {
            expect(computeCellState(1, 2)).to.equal(1);
            expect(computeCellState(1, 3)).to.equal(1);
        });

        it('should return 0 when cell is alive and has less than 2 or more than 3 alive neighbors', () => {
            expect(computeCellState(1, 1)).to.equal(0);
            expect(computeCellState(1, 4)).to.equal(0);
        });

        it('should return 0 when cell is dead and has not exactly 3 alive neighbors', () => {
            expect(computeCellState(0, 0)).to.equal(0);
            expect(computeCellState(0, 1)).to.equal(0);
            expect(computeCellState(0, 2)).to.equal(0);
        });
    });
    ...
    describe('countMooresNeighbours()', () => {
        it('should count the correct number of live neighbors', () => {
            ...
            const count = countMooresNeighbours(grid, width, x, y, widthMinOne, heightMinOne);

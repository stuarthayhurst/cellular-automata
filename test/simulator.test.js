import { describe, it, expect } from "vitest";

import {
    stepForward,
    conwaysCellState,
    countMooresNeighbours,
} from "../src/lib/simulation.js";
import { sharedState } from "../src/lib/sharedState.js";
import { reactiveState } from "../src/lib/reactiveState.svelte.js";

describe("Simulator Functions", () => {
    describe("Conway's cell state calculation", () => {
        it("should become alive with 3 alive neighbors", () => {
            expect(conwaysCellState(0, 3)).toBe(1);
        });

        it("should stay alive with 2 or 3 alive neighbors", () => {
            expect(conwaysCellState(1, 2)).toBe(1);
            expect(conwaysCellState(1, 3)).toBe(1);
        });

        it("should die with less than 2 or more than 3 alive neighbors", () => {
            expect(conwaysCellState(1, 1)).toBe(0);
            expect(conwaysCellState(1, 4)).toBe(0);
        });

        it("should stay dead with 0-2 alive neighbours", () => {
            expect(conwaysCellState(0, 0)).toBe(0);
            expect(conwaysCellState(0, 1)).toBe(0);
            expect(conwaysCellState(0, 2)).toBe(0);
        });
    });

    describe("Count Moore's Neighbours", () => {
        it("should correctly count the alive neighbours", () => {
            const grid = new Uint8Array([0, 1, 1, 0, 1, 0, 1, 1, 0]); // 3x3
            const width = 3;
            const x = 1;
            const y = 1;
            const count = countMooresNeighbours(grid, width, x, y);
            expect(count).toBe(4); // 4 live cells in the neighbors of position 1,1
        });
    });

    describe("Simulation Step", () => {
        it("should update the cells", () => {
            const initialCells = new Uint8Array([0, 1, 1, 0, 1, 0, 1, 1, 0]); // 3x3
            sharedState.cells = initialCells;
            reactiveState.cellGridWidth = 3; // weight

            stepForward();

            // Check if oldStateModelDontUse.cells has been updated
            expect(sharedState.cells).not.toBe(initialCells);
        });
    });
});
